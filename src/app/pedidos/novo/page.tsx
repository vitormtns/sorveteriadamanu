...
export default function NewOrderPage() {
  ...
  return <form onSubmit={submit} className="grid gap-5 xl:grid-cols-[1fr_380px]">
    <div className="grid gap-5">
      <Card><h2 className="mb-4 text-lg font-black">Cliente</h2><div className="grid gap-4 sm:grid-cols-2"><Field label="Nome do cliente"><Input name="customerName" required placeholder="Quem está pedindo?" /></Field><Field label="Telefone (opcional)"><Input name="phone" inputMode="tel" placeholder="(00) 00000-0000" /></Field></div></Card>
      <Card>
        <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-lg font-black">Escolha os produtos</h2><p className="text-sm text-slate-500">Toque em + para adicionar</p></div><div className="relative"><Search className="absolute left-3 top-3 text-slate-400" size={18} /><Input className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar..." /></div></div>
        <div className="grid gap-2 sm:grid-cols-2">
          {active.map((product) => { const quantity = quantities[product.id] || 0; return <div key={product.id} className={`flex items-center gap-3 rounded-2xl border p-3 transition ${quantity ? "border-purple-300 bg-purple-50" : "border-slate-100"}`}>
            <button type="button" onClick={() => change(product, 1)} className="min-w-0 flex-1 text-left"><p className="truncate font-bold text-slate-900">{product.name}</p><p className="text-sm font-bold text-[var(--purple)]">{formatCurrency(product.price)}</p></button>
            {quantity > 0 ? <div className="flex items-center gap-2"><button type="button" aria-label="Diminuir quantidade" className="grid h-9 w-9 place-items-center rounded-full bg-white shadow-sm" onClick={() => change(product, -1)}><Minus size={16} /></button><span className="w-5 text-center font-black">{quantity}</span><button type="button" aria-label="Aumentar quantidade" className="grid h-9 w-9 place-items-center rounded-full bg-[var(--purple)] text-white" onClick={() => change(product, 1)}><Plus size={16} /></button></div> : <button type="button" aria-label={`Adicionar ${product.name}`} className="grid h-10 w-10 place-items-center rounded-full bg-purple-100 text-purple-700" onClick={() => change(product, 1)}><Plus size={18} /></button>}
          </div>})}
        </div>
      </Card>
    ...
    <aside className="grid content-start gap-5">
      <Card className="xl:sticky xl:top-24">
        <h2 className="flex items-center gap-2 text-lg font-black"><ShoppingCart size={20} /> Resumo do pedido</h2>
        ...
        <div className="mb-5 flex items-center justify-between"><span className="font-bold">Total</span><span className="text-2xl font-black text-[var(--purple)]">{formatCurrency(total)}</span></div>
