...
export default function ProductsPage() {
  ...
  return <div className="grid gap-6">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-2xl font-black text-slate-900">Cardápio</h2><p className="text-sm text-slate-500">Cadastre e organize os produtos vendidos.</p></div><Button onClick={() => setEditing(null)}><Plus size={18} /> Adicionar produto</Button></div>
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {visible.map((product) => <Card key={product.id} className="flex items-center gap-4">
        <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-purple-100 text-purple-700"><Package size={22} /></div>
        <div className="min-w-0 flex-1"><p className="truncate font-bold text-slate-900">{product.name}</p><p className="text-sm text-slate-500">{product.category}</p><p className="mt-1 font-black text-[var(--purple)]">{formatCurrency(product.price)}</p></div>
    ...
    {editing !== undefined && <div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/40 sm:place-items-center" onMouseDown={(e) => e.target === e.currentTarget && setEditing(undefined)}>
      <form onSubmit={submit} className="w-full max-w-lg rounded-t-3xl bg-white p-6 shadow-2xl sm:rounded-3xl">
        <div className="mb-5 flex items-center justify-between"><h3 className="text-xl font-black">{editing ? "Editar produto" : "Novo produto"}</h3><button type="button" aria-label="Fechar" onClick={() => setEditing(undefined)}><X /></button></div>
