  ...
  return <div className="grid gap-6">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {visible.map((product) => <Card key={product.id} className="internal-metric group flex items-center gap-4">
        ...
        <div className="grid justify-items-end gap-2"><div className="flex flex-wrap justify-end gap-1"><span className={`rounded-full px-2 py-1 text-xs font-bold ${product.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{product.active ? "Ativo" : "Inativo"}</span><span className={`rounded-full px-2 py-1 text-xs font-bold ${product.availableToday ? "bg-sky-100 text-sky-700" : "bg-amber-100 text-amber-700"}`}>{product.availableToday ? "Disponível hoje" : "Indisponível hoje"}</span>{product.featured && <span className="rounded-full bg-purple-100 px-2 py-1 text-xs font-bold text-purple-700">Destaque</span>}</div><div><button aria-label={`Editar ${product.name}`} className="p-2 text-slate-500 hover:text-purple-700" onClick={() => setEditing(product)}><Edit3 size={17} /></button><button aria-label={`Excluir ${product.name}`} className="p-2 text-slate-500 hover:text-red-700" onClick={() => confirm("Deseja excluir este produto?") && deleteProduct(product.id)}><Trash2 size={17} /></button></div></div>
      </Card>)}
    </div>
