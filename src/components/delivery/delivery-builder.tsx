...
function Start({ choose, cart, checkout, availability, hasPromotions }: { choose: (kind: Kind) => void; cart: CartItem[]; checkout: () => void; availability: ReturnType<typeof getStoreAvailability>; hasPromotions: boolean }) {
  return <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 md:px-8 md:pt-16"><div className="max-w-2xl"><span className="inline-flex items-center gap-2 rounded-full bg-[#f1e4f1] px-3 py-2 text-xs font-bold uppercase tracking-[.12em] text-[#6a2478]"><Sparkles size={14} /> Feito por você</span><h1 className="mt-5 text-4xl font-extrabold leading-[1.02] tracking-[-.055em] text-[#24002f] md:text-6xl">O que você quer pedir?</h1><p className="mt-4 max-w-lg leading-relaxed text-[#706273]">Monte cada detalhe e envie o pedido direto para a equipe da Manu.</p></div>
    <div className={`mt-6 max-w-2xl rounded-2xl border p-4 text-sm ${availability.acceptingOrders ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-900"}`}><strong className="block">{availability.acceptingOrders ? "Estamos aceitando pedidos" : "Pedidos online indisponíveis"}</strong><span className="mt-1 block">{availability.message}</span></div>
