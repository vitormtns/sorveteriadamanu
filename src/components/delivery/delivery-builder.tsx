...
const formats = [{ name: "Copo", price: 0 }, { name: "Casquinha", price: 0 }, { name: "Pote", price: 2 }];
  ...
  return <main className="delivery-page min-h-screen bg-[#fbf7f0] text-[#190620]">
    ...
    {!kind ? <Start choose={setKind} cart={cart} checkout={() => { setKind("promo"); setStep(1); }} /> :
      <div className="mx-auto max-w-6xl px-4 pb-32 pt-5 md:px-8 md:pt-8">
        ...
        {building ? <div className="grid gap-6 md:grid-cols-[.9fr_1.1fr] md:items-start lg:gap-12"><div className="md:sticky md:top-28"><Preview kind={kind} format={format} shakeSize={shakeSize} flavors={selectedFlavors} extras={selectedExtras} topping={kind === "milkshake" ? shakeFlavor : iceExtra} /><p className="mt-3 text-center text-sm font-bold text-[#4a0b63]">{itemPrice ? formatCurrency(itemPrice) : "Monte do seu jeito"}</p></div><section className="rounded-[28px] border border-[#eadfea] bg-white p-5 shadow-[0_20px_70px_rgba(50,8,65,.08)] md:p-8">{kind === "acai" ? <AcaiStep step={step} size={size} setSize={setSize} selected={selectedExtras} toggle={name => setSelectedExtras(current => current.includes(name) ? current.filter(x => x !== name) : [...current, name])} note={acaiNote} setNote={setAcaiNote} /> : kind === "milkshake" ? <ShakeStep step={step} size={shakeSize} setSize={setShakeSize} flavor={shakeFlavor} setFlavor={setShakeFlavor} note={shakeNote} setNote={setShakeNote} /> : <IceStep step={step} format={format} setFormat={setFormat} scoop={scoop} setScoop={value => { setScoop(value); setSelectedFlavors([]); }} selected={selectedFlavors} select={selectFlavor} extra={iceExtra} setExtra={setIceExtra} />}</section></div>
...
function Preview({ kind, format, shakeSize, flavors: chosen, extras: chosenExtras, topping }: { kind: Kind; format: string; shakeSize: string; flavors: string[]; extras: string[]; topping: string }) {
  ...
  return <div className="delivery-preview relative mx-auto grid aspect-[4/3] max-h-[330px] w-full max-w-md place-items-center overflow-hidden rounded-[30px] bg-[radial-gradient(circle_at_50%_35%,#fff_0%,#f5e8ee_48%,#ead8e7_100%)]">
    ...
    {kind === "acai" ? <div className="acai-cup"><span className="acai-fill" /><span className="acai-top" />{chosenExtras.includes("Banana") && <span className="fruit fruit-banana" />}{chosenExtras.includes("Morango") && <span className="fruit fruit-strawberry" />}{chosenExtras.includes("Granola") && <span className="granola">••••••</span>}{(chosenExtras.includes("Leite condensado") || topping === "Leite condensado") && <span className="sauce sauce-light" />}{topping === "Chocolate" && <span className="sauce sauce-dark" />}{topping === "Morango" && <span className="sauce sauce-pink" />}</div>
      ...
      : <div className={`ice-vessel ${format === "Casquinha" ? "is-cone" : format === "Pote" ? "is-pot" : ""}`}><span className="vessel-base" /><span className="scoops">{chosen.map((flavor, i) => <i key={`${flavor}-${i}`} style={{ background: colors[flavor] }} />)}{chosen.length === 0 && <i />}</span>{topping && topping !== "Sem cobertura" && <span className={`ice-drip ${topping === "Morango" ? "pink" : topping === "Leite condensado" ? "light" : ""}`} />}</div>}
  </div>;
}
...
function Checkout({ cart, setCart, customer, setCustomer, error, submit, addMore }: { cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; customer: Customer; setCustomer: React.Dispatch<React.SetStateAction<Customer>>; error: string; submit: () => void; addMore: () => void }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[.85fr_1.15fr]">
    <section className="rounded-[28px] border border-[#eadfea] bg-white p-5 lg:sticky lg:top-28 lg:self-start"><Heading title="Seu pedido" text="Confira os itens antes de enviar." /><div className="grid gap-3">{cart.map(item => <div key={item.id} className="rounded-2xl bg-[#faf4f8] p-4"><div className="flex justify-between gap-4"><div><h3 className="text-sm font-extrabold">{item.name}</h3><p className="mt-1 text-xs leading-relaxed text-[#796b7b]">{item.detail}</p></div><button aria-label={`Remover ${item.name}`} onClick={() => setCart(current => current.filter(x => x.id !== item.id))} className="h-9 text-[#a44b70]"><Trash2 size={17} /></button></div><div className="mt-3 flex items-center justify-between"><div className="flex items-center gap-3"><button aria-label="Diminuir quantidade" onClick={() => setCart(current => current.map(x => x.id === item.id ? { ...x, quantity: Math.max(1, x.quantity - 1) } : x))}><Minus size={16} /></button><strong className="text-sm">{item.quantity}</strong><button aria-label="Aumentar quantidade" onClick={() => setCart(current => current.map(x => x.id === item.id ? { ...x, quantity: x.quantity + 1 } : x))}><Plus size={16} /></button></div><strong>{formatCurrency(item.price * item.quantity)}</strong></div></div>)}</div>
      {cart.length === 0 && <p className="rounded-2xl bg-[#faf4f8] p-5 text-sm text-[#796b7b]">Seu pedido está vazio. Adicione um item para continuar.</p>}<button onClick={addMore} className="mt-4 flex min-h-11 items-center gap-2 text-sm font-extrabold text-[#6d2779]"><Plus size={17} /> Adicionar outro item</button><div className="mt-4 flex justify-between border-t border-[#eadfea] pt-5 text-lg"><strong>Total estimado</strong><strong>{formatCurrency(total)}</strong></div>
    </section>
    <section className="rounded-[28px] border border-[#eadfea] bg-white p-5 md:p-8"><Heading title="Como você prefere receber?" text="Só precisamos de alguns dados para enviar o pedido." /><div className="grid gap-4"><Field label="Nome" value={customer.name} change={value => update({ name: value })} placeholder="Seu nome" /><Field label="Telefone" value={customer.phone} change={value => update({ phone: value })} placeholder="(00) 00000-0000" type="tel" />
      <fieldset><legend className="mb-2 text-sm font-bold">Tipo do pedido</legend><div className="grid grid-cols-2 gap-3"><DeliveryOption active={customer.deliveryType === "pickup"} action={() => update({ deliveryType: "pickup" })} icon={<PackageCheck />} title="Retirada" /><DeliveryOption active={customer.deliveryType === "delivery"} action={() => update({ deliveryType: "delivery" })} icon={<Truck />} title="Entrega" /></div></fieldset>
      ...
      {error && <p role="alert" className="mt-4 rounded-xl bg-[#fff0f3] p-3 text-sm font-bold text-[#a52d54]">{error}</p>}<button disabled={!cart.length} onClick={submit} className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#f8b900] px-5 font-extrabold text-[#24002f] disabled:opacity-40">Enviar pedido <ArrowRight size={19} /></button><p className="mt-3 text-center text-xs text-[#817284]">Seu pedido será enviado direto para a equipe da Manu.</p>
    </section>
  </div>;
}
function Field({ label, value, change, placeholder, type = "text" }: { label: string; value: string; change: SetText; placeholder: string; type?: string }) { return <label className="grid gap-2 text-sm font-bold">{label}<input type={type} value={value} onChange={e => change(e.target.value)} placeholder={placeholder} className="min-h-13 rounded-xl border border-[#dfd2df] px-4 font-normal outline-none focus:border-[#6d2779]" /></label>; }
function DeliveryOption({ active, action, icon, title }: { active: boolean; action: () => void; icon: React.ReactNode; title: string }) { return <button type="button" onClick={action} className={`flex min-h-20 items-center justify-center gap-2 rounded-2xl border text-sm font-extrabold ${active ? "border-[#6d2779] bg-[#f4eaf4] text-[#4a0b63]" : "border-[#dfd2df] text-[#766878]"}`}>{icon}{title}</button>; }
