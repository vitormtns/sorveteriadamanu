...
export function ContactSection() {
  ...
  return <section className="landing-mobile-section bg-[#fff8ef] px-5 py-20 md:px-8 md:py-28"><div className="mx-auto max-w-[1240px]"><AnimatedSection><p className="landing-kicker">Perto de você</p><h2 className="landing-title mt-3">Peça, retire ou receba.</h2></AnimatedSection><div className="contact-grid mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{contacts.map(({ icon: Icon, label, value }, index) => <AnimatedSection key={label} delay={index * 70}><div className="h-full rounded-2xl border border-[#ecdcd0] bg-white p-5"><Icon size={19} className="text-[#4a0b63]" /><p className="mt-5 text-xs font-medium text-slate-500">{label}</p><p className="mt-1 text-sm font-semibold text-[#16051f]">{value}</p></div></AnimatedSection>)}</div></div></section>;
}
