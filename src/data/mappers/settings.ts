...
import { DatabaseBusinessHour, DatabaseBusinessHourInsert, DatabasePublicStoreSettings, DatabaseStoreSettings, DatabaseStoreSettingsUpdate } from "@/data/supabase/database.types";
  ...
  return normalizeSettings({
    ...
    payments: {
      accepted: Object.fromEntries(
        Object.keys(initialSettings.payments.accepted).map((method) => [method, row.accepted_payment_methods.includes(method as keyof typeof initialSettings.payments.accepted)]),
      ) as StoreSettings["payments"]["accepted"],
      pixKey: row.pix_key,
...
export function mapPublicStoreSettingsFromDatabase(row: DatabasePublicStoreSettings, hours: DatabaseBusinessHour[] = []): StoreSettings {
  return mapStoreSettingsFromDatabase({
    ...
    free_add_ons_quantity: row.free_add_ons_quantity,
...
export function mapStoreSettingsUpdateToDatabase(settings: StoreSettings): DatabaseStoreSettingsUpdate {
  return {
    delivery_open: settings.status.deliveryOpen,
    pause_online_orders: settings.status.pauseOnlineOrders,
    temporary_pause: settings.status.temporaryPause,
    ...
    accepted_payment_methods: Object.entries(settings.payments.accepted)
      .filter(([, enabled]) => enabled)
      .map(([method]) => method) as DatabaseStoreSettingsUpdate["accepted_payment_methods"],
