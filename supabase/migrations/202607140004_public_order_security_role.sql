...
begin
  ...
  select * into v_existing from public.orders where origin = 'delivery' and idempotency_key = p_idempotency_key for update;
  if found then
  ...
  begin
    insert into public.orders (customer_name, phone, notes, payment_method, payment_status, order_status, origin, delivery_type, address, subtotal, delivery_fee, discount, total, idempotency_key, idempotency_payload_hash)
    values (v_customer_name, v_phone, v_notes, v_payment_method, 'pending', 'new', 'delivery', v_delivery_type, case when v_delivery_type = 'delivery' then v_address else null end, 0, 0, 0, 0, p_idempotency_key, v_payload_hash)
    returning * into v_order;
  exception when unique_violation then
    select * into v_existing from public.orders where origin = 'delivery' and idempotency_key = p_idempotency_key;
    if found then
      if v_existing.idempotency_payload_hash <> v_payload_hash then raise exception using errcode = '23505', message = 'IDEMPOTENCY_CONFLICT'; end if;
      return v_existing;
    end if;
  ...
  for v_item in select value from jsonb_array_elements(p_request -> 'items') loop
    ...
    v_unit_price := 0; v_name := ''; v_category := null; v_details := jsonb_build_object('builderType', v_builder_type);

    if v_builder_type = 'acai' then
      select * into v_size from public.delivery_builder_options where id = (v_item ->> 'size_id')::uuid and builder_type = 'acai' and option_type = 'size' and active and available;
      ...
      select coalesce(sum(case when position > v_settings.free_add_ons_quantity then extra_price else 0 end), 0), coalesce(jsonb_agg(jsonb_build_object('id', id, 'name', name, 'extraPrice', extra_price, 'included', position <= v_settings.free_add_ons_quantity) order by position), '[]'::jsonb) into v_add_on_paid_total, v_add_on_details from (select add_on.id, add_on.name, add_on.extra_price, row_number() over (order by add_on.display_order, add_on.id) as position from public.add_ons as add_on where add_on.id in (select value::uuid from jsonb_array_elements_text(coalesce(v_item -> 'add_on_ids', '[]'::jsonb))) and add_on.active and add_on.available) as selected_add_on;
      ...
      v_unit_price := round(v_size.price + v_add_on_paid_total, 2); v_name := 'Açaí ' || v_size.name; v_category := 'Açaí';
      ...
      select coalesce(jsonb_agg(jsonb_build_object('id', id, 'name', name, 'previewColor', preview_color) order by display_order, id), '[]'::jsonb) into v_flavor_details from public.flavors where id in (select value::uuid from jsonb_array_elements_text(v_item -> 'flavor_ids')) and product_type = 'ice_cream' and active and available;
      ...
      v_unit_price := round(v_format.price + v_scoop.price + v_topping.price, 2); v_name := 'Sorvete ' || lower(v_format.name) || ' - ' || v_scoop.name; v_category := 'Sorvetes';
      ...
      select coalesce(jsonb_agg(jsonb_build_object('id', id, 'name', name, 'previewColor', preview_color) order by display_order, id), '[]'::jsonb) into v_flavor_details from public.flavors where id in (select value::uuid from jsonb_array_elements_text(v_item -> 'flavor_ids')) and product_type = 'milkshake' and active and available;
      ...
      v_unit_price := v_size.price; v_name := 'Milk-shake ' || v_size.name; v_category := 'Milk-shakes';
      ...
      select * into v_promotion from public.promotions where id = (v_item ->> 'promotion_id')::uuid and active and price > 0 and (valid_from is null or valid_from <= now()) and (valid_until is null or valid_until >= now());
      if not found then raise exception using errcode = 'P0001', message = 'PROMOTION_UNAVAILABLE'; end if;
      v_unit_price := v_promotion.price; v_name := v_promotion.title; v_category := 'Promoções';
      ...
      v_unit_price := v_product.price; v_name := v_product.name; v_category := v_product.category;
    ...
    else raise exception using errcode = '22023', message = 'ITEMS_INVALID';
    end if;
    v_item_subtotal := round(v_unit_price * v_quantity, 2);
    insert into public.order_items (order_id, product_id, product_name, category, quantity, unit_price, subtotal, details, notes) values (v_order.id, case when v_builder_type = 'product' then v_product.id else null end, v_name, v_category, v_quantity, v_unit_price, v_item_subtotal, v_details, v_item_notes);
  ...
  update public.orders set subtotal = v_subtotal, delivery_fee = v_delivery_fee, discount = 0, total = round(v_subtotal + v_delivery_fee, 2) where id = v_order.id returning * into v_order;
  return v_order;
end;
$$;

revoke all on function public.consume_public_order_rate_limit(text, integer, integer) from public, anon, authenticated;
revoke all on function public.create_public_order(text, jsonb) from public, anon, authenticated;
...
comment on function public.create_public_order(text, jsonb) is 'Operação atômica e idempotente para pedidos públicos. Valida auth.role() = service_role antes de operar.';
